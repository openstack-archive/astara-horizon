...
class RugClient(object):
    def __init__(self):
        self.host = _get_local_service_ip(settings.RUG_MANAGEMENT_PREFIX).split('/')[0]
    ...
    def router_rebuild(self, router_id, router_image_uuid=None):
        if router_image_uuid:
            path = '/router/rebuild/{router_id}/--router_image_uuid/{router_image_uuid}'.format(
                router_id=router_id,
    ...
    def get_routers(self, request, **search_opts):
        ...
        for router in neutronclient(request).list_routers(**search_opts).get("routers", []):
            instances = nova_client.servers.list(True, search_opts={'name': 'ak-' + router['id'],
                                                                    'all_tenants': True})
            instance = instances[0] if instances else None
            image = nova_client.images.get(instance.image['id']) if instance else None
